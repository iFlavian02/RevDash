...
Scans are engine-owned serialized operations. Streaming drains before queued Mode 03 and Mode 07 requests run, and resumes only after the diagnostic queue completes. Results preserve ECU provenance, deduplicate only equivalent records from the same ECU/status category, optionally enrich records through the configured read-only DTC database, and request a supported Mode 02 frame-zero RPM sample when a DTC is present. Identification similarly combines VIN, CALID, and CVN records per ECU while retaining useful partial results when an ECU does not support every Mode 09 PID.

Mode 04 is a two-phase same-engine workflow. Preparation is permitted only for a ready physical source with no competing diagnostic operation and a valid, non-future vehicle-speed sample no older than the Mode 01 speed stale threshold; speed must be at most 0.5 km/h. The preparation captures current DTC/freeze-frame/finding evidence, the engine epoch, source configuration, all known VINs, a destructive-effects warning, and a random single-use token that expires after 30 seconds.

Confirmation consumes the token on the engine worker, then revalidates source identity, epoch, vehicle identity, readiness, speed quality/freshness, and stationary state immediately before dispatch. Source changes, disconnects, identification/scans, epoch changes, expiry, and failed revalidation invalidate the preparation. A successful `0x44` response is followed by a bounded settling interval and an automatic Mode 03/07 rescan. Every transmitted clear attempt, including negative responses, timeouts, disconnect cancellation, and post-clear rescan failure, produces an immutable in-memory `Mode04AuditRecord`; Stage 6 persists these records in sessions. Synthetic and playback sources remain blocked from user-facing clear even though the synthetic protocol fixture supports Mode 04 for lower-level tests.
...
## Offline DTC database

Licensed DTC data enters the application only through the offline importer. Its CSV contract contains `code`, `description`, `severity`, pipe-separated `likely_failure_points`, and `source_version`; all rows must use one non-empty source version. Import validates RFC-style CSV quoting, UTF-8, SAE code shape, required values, supported severity values, and duplicate/conflicting codes before atomically publishing a database.

Generated SQLite databases carry an application ID, schema version, source version, and an embedded `production` or `fixture` identity. Runtime access is read-only. Exact code lookup, indexed code-prefix lookup, and normalized token-prefix keyword search share the same immutable definitions; a valid but absent code produces an explicit unknown fallback rather than a database error.

Fixture identity is a safety boundary, not a filename convention. Runtime callers must state the expected database identity, and production is the default. A fixture database cannot be opened as production even if copied or renamed. Release configuration must set `REVDASH_REQUIRE_PRODUCTION_DTC_DATABASE` with an explicit `REVDASH_PRODUCTION_DTC_DATABASE`; the build gate verifies the database's embedded production identity. Licensed production CSV and generated database content remain external to source control, while tests generate temporary SQLite databases from the legal fixture CSV.

## Session recording

Schema v1 sessions are streaming UTF-8 JSON Lines. Every line carries a record type, schema version, and a non-negative integer `elapsed_us` measured from the recording's monotonic origin. The header captures UUID, application and schema versions, UTC start, active source, available adapter/protocol and vehicle metadata, and synthetic seed/configuration when applicable. Canonical logical OBD payloads are uppercase hexadecimal, and ECU numeric value plus address format are preserved. Telemetry, DTCs/freeze frames, heuristic findings, Mode 04 audits, ECU metadata, and explicit recorder-queue loss markers have separate record types.
