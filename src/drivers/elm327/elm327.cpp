...
struct Elm327DataSource::Command { std::string text; bool optional; std::chrono::steady_clock::time_point started; std::function<void(core::Result<ElmResponse>)> completion; };
...
void Elm327DataSource::initialize(std::size_t index, core::CompletionCallback completion) {
    ...
    runCommand(std::string{commands[index]}, optional, [this, index, completion = std::move(completion), optional](core::Result<ElmResponse> result) mutable { if (!result && !optional) { completion(tl::make_unexpected(result.error())); return; } if (result && commands[index] == "ATI" && !result->lines.empty()) stats_.adapter_identity = result->lines.front(); initialize(index + 1, std::move(completion)); });
}
...
void Elm327DataSource::startTransmit(const core::ObdRequest& request, core::CompletionCallback completion) { runCommand(requestText(request), false, [this, completion = std::move(completion)](core::Result<ElmResponse> result) mutable { if (!result) { completion(tl::make_unexpected(result.error())); return; } normalize(*result); completion(core::makeSuccess()); }); }
void Elm327DataSource::runCommand(std::string command, bool optional, std::function<void(core::Result<ElmResponse>)> completion) {
    ...
    parser_.reset(); active_command_ = std::make_unique<Command>(Command{command, optional, std::chrono::steady_clock::now(), std::move(completion)});
...
void Elm327DataSource::finishCommand(core::Result<ElmResponse> result) { if (!active_command_) return; auto command = std::move(active_command_); if (result) { const auto command_text = upper(command->text); result->lines.erase(std::remove_if(result->lines.begin(), result->lines.end(), [&command_text](const std::string& line) { return upper(line) == command_text; }), result->lines.end()); const auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - command->started); stats_.last_rtt = elapsed; stats_.ewma_rtt = stats_.ewma_rtt.count() == 0 ? elapsed : std::chrono::milliseconds((stats_.ewma_rtt.count() * 7 + elapsed.count()) / 8); for (const auto& line : result->lines) raw_lines_.push_back(line); if (result->status == ElmResponseStatus::UnsupportedCommand && !command->optional) result = tl::make_unexpected(malformed("Unsupported required AT command")); } else if (result.error().code == core::toString(core::ErrorCode::TransportTimeout)) ++stats_.timeout_count; if (command->completion) command->completion(std::move(result)); }
